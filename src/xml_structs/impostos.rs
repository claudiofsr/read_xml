...
pub trait ConvertToOptionValue<T> {
    fn to_some_value(&self) -> Option<T>;
}

impl<T> ConvertToOptionValue<T> for Option<String>
where
    T: std::str::FromStr
{
    /// Convert `Option<String>` to `Option<T>`
    fn to_some_value(&self) -> Option<T> {
        self
            .as_ref()
            .and_then(|string| {
...
impl Imposto {
    ...
    pub fn get_v_bc_icms(&self) -> Option<f64> {
        self
            .icms
            .as_ref()
            .and_then(|imposto_de_circulacao| {
                imposto_de_circulacao
                    .get_icms_valor_base_calc()
            })
    }
    ...
    pub fn get_v_iss(&self) -> Option<f64> {
        self
            .issqn
            .as_ref()
            .and_then(|imposto_sobre_servico| {
    ...
    pub fn get_v_ipi(&self) -> Option<f64> {
        self
            .ipi
            .as_ref()
            .and_then(|imposto_sobre_produto| {
                imposto_sobre_produto
                    .iter()
                    .map(|tributo| {
                        tributo.get_valor_ipi()
                    })
                    .sum()
