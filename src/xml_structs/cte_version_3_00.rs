...
pub fn serialize_tomador_do_servico<S>(codigo: &Option<u8>, serializer: S) -> Result<S::Ok, S::Error>
where
    S: Serializer,
{
    if codigo.is_none() {
...
impl CteProc {
   ...
   pub fn get_recebedor_ender_municipio(&self) -> Option<String> {
    ...
    pub fn get_cte_complementar(&self) -> Vec<String> {
        let cte_multimodal: Vec<String> = self
            .get_cte_multimodal();
   ...
   /// Recebedor: CNPJ
    ...
    fn get_cte_comp(&self) -> Vec<String> {
        self
            .inf_cte_comp
            .iter()
            .flat_map(|info_complementar| {
                info_complementar.merge_keys()
            })
            .collect()
    }
...
pub trait GetDocs {
    ...
    fn merge_keys(&self) -> Vec<String> {
        ...
        let docs: Vec<String> = [ch_cte, chave]
            .into_iter()
            .flatten()
            .collect();
