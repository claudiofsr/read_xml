...
pub use self::{
    ...
    xml_structs::{
        ...
        cte_evento::{ProcEventoCte, InfoCteEvento},
        cancelamento_cte::{ProcCancCte, InfoCteCancel},
        cancelamento_nfe::{ProcCancNfe, InfoNfeCancel},
        nfe_version_4_00::{NfeProc,InfoNfe},
        nfe_evento::{ProcEventoNfe, InfoNfeEvento},
...
use std::{
    ...
    fmt::{self, Debug}, fs::File, hash::Hash,
    io::{BufReader, Write}, ops::Deref,
    path::{Path, PathBuf},
    process::exit, str,
...
impl MultiProgressBar {
    fn add_progressbar(&mut self, multi_progress: &MultiProgress, name: &str, total: usize) -> MyResult<ProgressBar> {
...
impl Information {
    ...
    pub fn add_info_to_docs_fiscais(&self, docs_fiscais: &mut DocsFiscais) {
        match self {
            ...
            Information::CancelamentoCte(info_cte_cancel) => docs_fiscais.cancel_cte.push(*info_cte_cancel.clone()),
            Information::CancelamentoNfe(info_nfe_cancel) => docs_fiscais.cancel_nfe.push(*info_nfe_cancel.clone()),
...
impl DocsFiscais {
    ...
    pub fn total(&self) -> usize {
        [self.ctes.is_empty(), self.nfes.is_empty(), self.efinanceiras.is_empty()]
            .into_iter()
    ...
    fn get_ctes_nao_encontrados(&self, cte_info: &BTreeMap<KeyDoc, Vec<InfoCte>>) -> HashSet<String> {
        self
            .ctes
            .par_iter()
            .flat_map(|info| {
    ...
    fn get_nfes_nao_encontrados(&self, nfe_info: &BTreeMap<KeyDoc, Vec<InfoNfe>>) -> HashSet<String> {
        self
            .ctes
            .par_iter()
            .flat_map(|info| {
    ...
    fn groupby_cte_ctes(&self, cte_info: &BTreeMap<KeyDoc, Vec<InfoCte>>) -> HashMap<String, HashSet<String>> {
        self
            .ctes
            .par_iter()
            .filter_map(|info| {
    ...
    fn groupby_cte_nfes(&self, nfe_info: &BTreeMap<KeyDoc, Vec<InfoNfe>>) -> HashMap<String, HashSet<String>> {
        self
            .ctes
            .par_iter()
            .filter_map(|info| {
    ...
    pub fn get_correlations(&mut self, arguments: &Arguments) {
        ...
        let correlacoes = Correlacoes { cte_info, nfe_info, cte_nfes, nfe_ctes };
            ...
            .for_each(|info| {
                if let Some(cte) = &info.cte {
                    if let Some(nfes) = correlacoes.cte_nfes.get(cte) {
                        ...
                        info.ncm_descricao = get_nfes_grouped_by_ncm_description(nfes, &correlacoes.nfe_info, arguments);
            ...
            .for_each(|info| {
                if let Some(nfe) = &info.nfe {
                    if let Some(ctes) = correlacoes.nfe_ctes.get(nfe) {
                        ...
                        info.tomadores = get_ctes_grouped_by_payer(ctes, &correlacoes.cte_info, arguments);
...
pub fn print_cte_nfes(correlacoes: &Correlacoes, arguments: &Arguments) {
    println!("cte_nfes:");
    correlacoes.cte_nfes.iter().sorted_by_key(|tuple| tuple.0).for_each(|(cte, nfes)| {
        println!("{cte}: {:>2} {:?} ; Valor Total = {} ; NCMs: {:?}",
            nfes.len(),
            nfes.to_vec_sorted(),
...
pub fn print_nfe_ctes(correlacoes: &Correlacoes, arguments: &Arguments){
    println!("nfe_ctes:");
    correlacoes.nfe_ctes.iter().sorted_by_key(|tuple| tuple.0).for_each(|(nfe, ctes)| {
        println!("{nfe}: {:>2} {:?} ; Valor Total = {} ; Tomadores: {:?}",
            ctes.len(),
            ctes.to_vec_sorted(),
        ...
        loop {
            ...
            for (cte_a, ctes) in self.iter() {
                for cte_complementar in ctes.iter().filter(|&cte| cte != cte_a) {
                    ...
                    to_add.entry(cte_complementar.clone()).or_default().insert(cte_a.clone());
...
fn get_total_value_ctes(ctes: &HashSet<String>, cte_info: &BTreeMap<KeyDoc, Vec<InfoCte>>) -> Option<f64> {
    ctes
        .par_iter()
        .map(|cte| {
            let chave_valida = KeyDoc::new(cte, true);
...
fn get_total_value_nfes(nfes: &HashSet<String>, nfe_info: &BTreeMap<KeyDoc, Vec<InfoNfe>>) -> Option<f64> {
    nfes
        .par_iter()
        .map(|nfe| {
            let chave_valida = KeyDoc::new(nfe, true);
        ...
        .flat_map(|nfe| {
            ...
            nfe_info.get(&chave_valida).map(|infos| {
                let keys: Vec<(NFeKeys, f64)> = infos
                    .iter()
                    .flat_map(|info| {
                        match (&info.ncm, &info.descricao, &info.v_prod) {
                ...
                let keys: Vec<(CTeKeys, f64)> = infos
                    ...
                    .flat_map(|info| {
                        ...
                        let opt_tomador_atributo = TOMADOR_DO_SERVICO
                            .get(&info.tomador_codigo).map(|&t| t.to_string());
                        match (opt_tomador_cnpj_cpf, opt_tomador_atributo, &info.valor_total) {
                            (Some(tomador_cnpj_cpf), Some(tomador_atributo), Some(valor)) if *valor > 0.0 => {
                                let ctekeys = CTeKeys {
                                    tomador_cnpj_cpf,
                                    tomador_atributo,
                                };
...
pub fn adicionar_eventos_nfe(
    ...
    let eventos_agrupados_por_chaves: HashMap<String, Vec<&InfoNfeEvento>> = eventos_nfe
        .group_by_hashmap_key_vector_v2();

    let cancela_agrupados_por_chaves: HashMap<String, Vec<&InfoNfeCancel>> = cancelamentos_nfe
        .group_by_hashmap_key_vector_v2();
...
pub fn adicionar_eventos_cte(
    ...
    let eventos_agrupados_por_chaves: HashMap<String, Vec<&InfoCteEvento>> = eventos_cte
        .group_by_hashmap_key_vector_v2();

    let cancela_agrupados_por_chaves: HashMap<String, Vec<&InfoCteCancel>> = cancelamentos_cte
        .group_by_hashmap_key_vector_v2();
    ...
    */
    ...
    fn correlacionar_ctes() -> MyResult<()> {
        ...
        let mut cte_ctes: HashMap<String, HashSet<String>> = HashMap::from([
            ("cte_a".to_string(), HashSet::from(array_a.map(String::from))),
            ("cte_b".to_string(), HashSet::from(array_b.map(String::from))),
            ("cte_c".to_string(), HashSet::from(array_c.map(String::from))),
            ("cte_d".to_string(), HashSet::from(array_d.map(String::from))),
        ]);

        println!("cte_ctes: [Start]");
        cte_ctes.iter().for_each(|(k, chaves)| println!("{k}: {chaves:?}"));
        ...
        assert_eq!(
            cte_ctes,
            HashMap::from([
                ("cte_1".to_string(), HashSet::from(array_1.map(String::from))),
                ("cte_2".to_string(), HashSet::from(array_2.map(String::from))),
                ("cte_3".to_string(), HashSet::from(array_3.map(String::from))),
                ("cte_4".to_string(), HashSet::from(array_4.map(String::from))),
                ("cte_5".to_string(), HashSet::from(array_5.map(String::from))),
                ("cte_6".to_string(), HashSet::from(array_6.map(String::from))),
                ("cte_7".to_string(), HashSet::from(array_7.map(String::from))),
                ("cte_a".to_string(), HashSet::from(array_a.map(String::from))),
                ("cte_b".to_string(), HashSet::from(array_b.map(String::from))),
                ("cte_c".to_string(), HashSet::from(array_c.map(String::from))),
                ("cte_d".to_string(), HashSet::from(array_d.map(String::from))),
    ...
    fn expand_cte_nfes() -> MyResult<()> {
        ...
        let mut cte_nfes: HashMap<String, HashSet<String>> = HashMap::from([
            ("cte_1".to_string(), HashSet::from(array_1.map(String::from))),
            ("cte_3".to_string(), HashSet::from(array_2.map(String::from))),
            ("cte_8".to_string(), HashSet::from(array_3.map(String::from))),
        ]);
        ...
        println!("cte_nfes:");
        cte_nfes.iter().for_each(|(cte, nfes)| {
            println!("{cte}: {:>2} {nfes:?}", nfes.len())
        });
        println!("cte_nfes.len(): {}\n", cte_nfes.len());
        ...
        assert_eq!(
            cte_nfes,
            HashMap::from([
                ("cte_1".to_string(), HashSet::from(array_x.map(String::from))),
                ("cte_2".to_string(), HashSet::from(array_x.map(String::from))),
                ("cte_3".to_string(), HashSet::from(array_x.map(String::from))),
                ("cte_4".to_string(), HashSet::from(array_x.map(String::from))),
                ("cte_8".to_string(), HashSet::from(array_y.map(String::from))),
                ("cte_a".to_string(), HashSet::from(array_x.map(String::from))),
                ("cte_b".to_string(), HashSet::from(array_x.map(String::from))),
            ])
        );
